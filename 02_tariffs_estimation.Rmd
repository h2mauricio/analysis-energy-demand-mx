...
df.enc.dwell$bill.period1[df.enc.dwell$bill.days1.valid <= 35] <- 1
df.enc.dwell$bill.period1[df.enc.dwell$bill.days1.valid >= 56] <- 2
...
df.enc.dwell$bill.period2[df.enc.dwell$bill.days2.valid <= 35] <- 1
df.enc.dwell$bill.period2[df.enc.dwell$bill.days2.valid >= 56] <- 2
...
In average, Mexican households paid ... 
...
pal.bulbs <- c('blue', 'orange', 'red')
pal.bulbs <- setNames(pal.bulbs, c("low", "med", "high"))

pl.cons.month.dwell.box <- plot_ly(type = 'box') %>%
  add_boxplot(y = df.enc.dwell.bill$cons.month.kwh.low1, 
              boxpoints = 'outliers', 
              name = "Low", 
              color = list(color =pal.bulbs['low']),
              marker = list(color = pal.bulbs['low']), 
              line=list(color = pal.bulbs['low']) ) %>%  
  add_boxplot(y = df.enc.dwell.bill$cons.month.kwh.med1, 
              boxpoints = 'outliers', 
              name = "Medium", 
              color = list(color =pal.bulbs['med']),
              marker = list(color = pal.bulbs['med']), 
              line=list(color = pal.bulbs['med']) ) %>%
  add_boxplot(y = df.enc.dwell.bill$cons.month.kwh.hig1, 
              boxpoints = 'outliers', 
              name = "High", 
              color = list(color =pal.bulbs['high']),
              marker = list(color = pal.bulbs['high']), 
              line=list(color = pal.bulbs['high'])) %>%
...
knitr::kable(stat.month.kwh.socioec[order(stat.month.kwh.socioec$mean), ], 
      ...
      caption = 'Summary statistics of the monthly electricity consumption by socioeconomic status') %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed", "responsive"),
                fixed_thead = T)
```
...
df.bill <- subset(df.enc.dwell.bill,
                   select=c(folio, cons_med1, region.tariff, electri, 
                            ...
                            year.end1, bill.days1, bill.days1.valid, 
